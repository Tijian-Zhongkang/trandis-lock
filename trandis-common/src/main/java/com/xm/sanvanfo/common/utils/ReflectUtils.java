...
public class ReflectUtils {
    ...
    private static final Pattern DESC_PATTERN = Pattern.compile(DESC_REGEX);
    ...
    public static Object getPropertyByFieldName(Object obj, String field) {
        for (Class<?> clazz = obj.getClass();
             clazz != null && !clazz.equals(Object.class);
             clazz = clazz.getSuperclass()) {
            try {
    ...
    public static Object getStaticPropertyByFiledName(Class clazz, String field) {
        for (;
             clazz != null && !clazz.equals(Object.class);
             clazz = clazz.getSuperclass()) {
            try {
    ...
    public static void setPropertyByFieldName(Object obj, Object o, String field) {
        for (Class<?> clazz = obj.getClass();
             clazz != null && !clazz.equals(Object.class);
             clazz = clazz.getSuperclass()) {
            try {
        ...
        throw new RuntimeException("该类不包含属性：" + field);
    }
    ...
    public static Method getDeclareMehodByName(Class<?> clazz, String methodName, Class<?> ... param) throws NoSuchMethodException {
        Class<?>cls = clazz;
        for (;cls!= Object.class;cls = cls.getSuperclass()) {
            try {
                return  cls.getDeclaredMethod(methodName, param);
            }
            catch (NoSuchMethodException ignore){

            }
        }
        throw new NoSuchMethodException(String.format("方法：%s没有找到", methodName));
    }


    public static List<Method> getAbstractMethod(Class<?> clazz) {
    ...
    public static void getAssociatedClass(Class<?> clazz, HashSet<Class> hashSet, String... filters) {
       if(hashSet == null) {
           throw new RuntimeException("hash set can't be null");
       }
       if(null == clazz) {
           return;
       }
       ...
       if(StringUtils.isEmpty(clazz.getCanonicalName())) {
           return;
       }
        ...
        if(null != filters) {
            for (String filter : filters) {
                if (clazz.getCanonicalName().startsWith(filter)) {
                    ret = true;
                    break;
                }
            }
       ...
       if(hashSet.contains(clazz)) {
        ...
        while(clazz != null  && clazz != Object.class) {
            try {
                ...
                Optional.ofNullable(fields).ifPresent(o -> {
                    for (Field field : o
                        ...
                        if (!type.isPrimitive()) {
                            getAssociatedClass(type, hashSet, filters);
                        }
                    }
                });
                ...
                Optional.ofNullable(methods).ifPresent(o -> {
                    for (Method method : o) {
                        ...
                        if (!returnType.isPrimitive()) {
                            getAssociatedClass(returnType, hashSet, filters);
                        }

                        Class<?>[] parameterTypes = method.getParameterTypes();
                        Optional.ofNullable(parameterTypes).ifPresent(q -> {
                            for (Class parameterType : q
                            ) {
                                if (parameterType.isPrimitive()) {
                                    getAssociatedClass(parameterType, hashSet, filters);
                                }
                            }
                        });
                    }
            ...
            if(!clazz.isInterface()) {
                clazz= clazz.getSuperclass();
                getAssociatedClass(clazz, hashSet, filters);
            }
            else {
                break;
            }
    ...
    public static Class<?>[] getInterfaces(Class cls) {
        ...
        for(;cls != Object.class; cls = cls.getSuperclass()) {
            CollectionUtils.addAll(classes, cls.getInterfaces());
        }
